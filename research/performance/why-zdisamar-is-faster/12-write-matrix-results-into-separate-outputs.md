...
Source link: [DISAMAR GitLab source](https://gitlab.com/KNMI-OSS/disamar/disamar/-/blob/d17c52884a875cb87b98e4c4ea7f722659e685ac/src/LabosModule.f90#L3528-L3569)
...
function smul(nmutot, nGauss, thresholdMul, a, b)
  ...
  ! SLOW: returning a matrix value means the function materializes a result
  !       array, and the caller uses that array inside the surrounding
  !       expression.
...
Source link: [DISAMAR GitLab source](https://gitlab.com/KNMI-OSS/disamar/disamar/-/blob/d17c52884a875cb87b98e4c4ea7f722659e685ac/src/LabosModule.f90#L2199-L2202)
...
!       so each line walks the matrix several times and materializes
!       intermediate arrays.
...
//       no return-by-value temporary, and `noalias` lets the compiler keep
//       `a` and `b` rows in registers because it knows `out` cannot alias
//       them.
...
There are two related ideas here. The first is avoiding temporary result arrays in the repeated loop:

```python
# Slow: function returns a separate result array, caller assigns it
def smul(a, b):
    out = [0.0] * n
    for i in range(n):
        out[i] = a[i] * b[i]
...
x = smul(a, b)        # materialize a result every call
y = smul(a, b)        # again

# Fast: caller passes the destination directly
