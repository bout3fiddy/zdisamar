...
- zdisamar
  - [Inverse forward call](https://github.com/bout3fiddy/zdisamar/blob/aa3bdc776e605229b18b54a7999632fb276546e2/python/zdisamar/inverse_method/optimal_estimation/o2a.py#L95-L122): passes the actual state-vector Jacobian names into the forward call.
  - [Runtime state selection](https://github.com/bout3fiddy/zdisamar/blob/aa3bdc776e605229b18b54a7999632fb276546e2/python/zdisamar/runtime.py#L113-L145): maps requested names to backend state IDs.
...
surface albedo is not in the retrieval state, the forward model should not
return or carry a surface-albedo Jacobian column for that retrieval.

```python
# Broad route: always calculate every supported derivative.
all_names = ["aod", "mid_pressure", "surface_albedo"]
y, jacobian = forward_model(jacobian=True, jacobian_state_names=all_names)
...
y, k = forward_model(jacobian=True, jacobian_state_names=state_names)
