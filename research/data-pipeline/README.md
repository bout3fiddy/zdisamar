...
- `src/forward_model/calculation_telemetry.zig` is a tiny facade at the math call
  sites.
- Normal library, CLI, test, benchmark, and Tracy trace modules import
  `src/forward_model/calculation_telemetry_stub.zig`; `Telemetry.enabled` is a
...
- The real sink lives under `src/validation/performance/`, not under
...
```sh
uv run python research/data-pipeline/generate_calculation_parquet.py
```
...
base = Path("research/data-pipeline/data/o2a-default")
