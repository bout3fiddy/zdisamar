...
const VerticalInterval = @typeInfo(@TypeOf(@as(Scene, .{}).atmosphere.interval_grid.intervals)).pointer.child;
...
fn prepareConfiguredVendorO2ATraceCaseWithProfile(
    ...
    const route = try prepareConfiguredVendorO2ARoute(&scene, configuredVendorO2ARtmControls(config));
    if (preparation_profile) |profile| recordPreparationLap(
        if (phase_timer) |*timer| timer else null,
...
fn prepareConfiguredVendorO2ARoute(
    scene: *const Scene,
