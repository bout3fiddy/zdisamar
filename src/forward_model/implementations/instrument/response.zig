...
pub fn defaultKernelHalfSpanNm(fwhm_nm: f64) f64 {
    ...
    // math: half_span = max(3 * max(FWHM, 1e-4), 1e-4).
...
pub fn writeIdentityKernel(kernel: *types.IntegrationKernel, enabled: bool) void {
    ...
    // math: identity integration is y(lambda_i) = 1 * y(lambda_i + 0).
...
//   math: response(delta) is one of Gaussian-modulated, flat-top N4, triple flat-top N4, or builtin table shape
pub fn spectralResponseWeight(response: InstrumentModel.SpectralResponse, offset_nm: f64) f64 {
    ...
    return switch (response.slit_index) {
        .gaussian_modulated => {
            ...
            // math: gaussian = exp(-0.5 * (delta / sigma)^2), sigma = FWHM / 2.354820045.
            ...
            // math: modulation = 1 + amplitude * sin(scale * delta / FWHM + phase)^2.
...
pub fn builtinLineShapeWeight(shape: BuiltinLineShapeKind, fwhm_nm: f64, offset_nm: f64) f64 {
    ...
    return switch (shape) {
        .gaussian => {
            ...
            // math: gaussian(delta) = exp(-0.5 * (delta / sigma)^2).
...
pub fn flatTopN4Weight(fwhm_nm: f64, offset_nm: f64) f64 {
    ...
    // math: flat_top_n4(delta) = 2^(-2 * (delta / max(w, 1e-6))^4), w = FWHM / 1.681793.
