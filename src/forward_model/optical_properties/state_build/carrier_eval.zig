...
fn fillSourceInterfaceFromBoundaryParts(
    ...
    // an interface. The caller chooses whether Rayleigh P2(lambda) was cached or should be computed here.      |
    //                                                                                                          |
    // math                                                                                                     |
    //   k_sca_above = k_sca_gas + k_sca_particle_above                                                         |
    ...
    //   prepared aerosol phase coefficient array owned by PreparedOpticalState.                                |
    // -------------------------------------------------------------------------------------------------------- |
...
fn fillRtmQuadratureLevelFromBoundaryParts(
    ...
    // particle carriers. Source-function integration reads ksca, phase weights, and optional aerosol           |
    // Jacobian slots from this row.                                                                            |
    //                                                                                                          |
    // math                                                                                                     |
...
fn weightedSpectroscopyEvaluationAtSupportRow(
    ...
    var weighted = SpectroscopyState.zeroSpectroscopyEvaluation();
    ...
    if (skip_o2_line_absorbers and sublayer.oxygen_number_density_cm3 > 0.0) {
        ...
        SpectroscopyState.addWeightedSpectroscopyEvaluation(
            &weighted,
            o2_evaluation,
    ...
    for (self.line_absorbers) |line_absorber| {
        ...
        total_weight += weight;
        SpectroscopyState.addWeightedSpectroscopyEvaluation(&weighted, evaluation, weight);
    }

    if (total_weight <= 0.0) return weighted;
    SpectroscopyState.normalizeSpectroscopyEvaluation(&weighted, total_weight);
    return weighted;
}
