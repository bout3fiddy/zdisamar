...
pub fn solveWithEvaluator(
    ...
    const anchor = try synthetic_forward.anchorStateWithLayout(allocator, problem, .oe, observed, layout);
    ...
    while (iterations < max_iterations) : (iterations += 1) {
        const predicted = synthetic_forward.featureVector(
            try synthetic_forward.summarizeStateWithLayout(problem, .oe, state, evaluator, layout),
            .oe,
        );
    ...
    const fitted_summary = try synthetic_forward.summarizeStateWithLayout(problem, .oe, state, evaluator, layout);
