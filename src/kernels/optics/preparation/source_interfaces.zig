...
test "shared weighted source interfaces keep above and below particle carriers distinct" {
    ...
    try std.testing.expect(source_interfaces[1].rtm_weight > 0.0);
    try std.testing.expectApproxEqAbs(0.2, source_interfaces[1].particle_ksca_below, 1.0e-12);
    try std.testing.expectApproxEqAbs(0.4, source_interfaces[1].particle_ksca_above, 1.0e-12);
    try std.testing.expectApproxEqAbs(source_interfaces[1].gas_ksca + 0.4, source_interfaces[1].ksca_above, 1.0e-12);
    try std.testing.expectApproxEqAbs(source_interfaces[1].gas_ksca + 0.2, source_interfaces[1].ksca_below, 1.0e-12);
    try std.testing.expect(@abs(source_interfaces[1].phase_coefficients_above[1] - source_interfaces[1].phase_coefficients_below[1]) > 1.0e-6);
}
