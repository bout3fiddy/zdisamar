...
pub const TransmissionProbe = struct {
...
pub const max_transmission_iteration_probes: usize = 60;

pub const TransmissionIterationProbe = struct {
    iteration: usize,
    b_before: f64,
    q_value: f64,
    d_value: f64,
...
fn doDoubleWithProbe(
    ...
    T: *basis.Mat,
    E: *basis.Vec,
    probes: *[max_transmission_iteration_probes]TransmissionIterationProbe,
) usize {
    var count: usize = 0;
    var b = b_start;
    for (0..ndouble) |iteration| {
        const Q = basis.qseries(n, n_gauss, threshold_mul, R, R);
        const qe = basis.semul(n, &Q, E);
        const qt = basis.smul(n, n_gauss, threshold_mul, &Q, T);
        var D = basis.matAdd(n, T, &qe);
        ...
        const t_before = T.get(row_index, column_index);
        const ed = basis.esmul(n, E, &D);
        const te = basis.semul(n, T, E);
        const td = basis.smul(n, n_gauss, threshold_mul, T, &D);
        var T_new = basis.matAdd(n, &ed, &te);
        ...
        if (count < probes.len) {
            probes[count] = .{
                ...
                .b_before = b,
                .q_value = Q.get(row_index, column_index),
                .d_value = D.get(row_index, column_index),
                .u_value = U.get(row_index, column_index),
                .t_before = t_before,
                .t_after = T.get(row_index, column_index),
            };
            count += 1;
        }
...
pub fn calcTransmissionProbe(
    ...
    if (use_doubling and i_fourier == 0 and controls.renorm_phase_function) {
    ...
    const row_mu = geo.u[row_index];
    const col_mu = geo.u[column_index];
    const du = row_mu - col_mu;
    const eet = if (@abs(du) < 1.0e-6)
        b_start * E.get(row_index)
    else
        E.get(row_index) - E.get(column_index);
    const dmu_min = if (@abs(du) < 1.0e-6)
        0.25 / @max(row_mu * col_mu, 1.0e-12)
    else
        0.25 / du;
    const zplus_value = z.Zplus.get(row_index, column_index);
    const single_t_value = a * zplus_value * eet * dmu_min;
    ...
    return .{
        ...
        .max_beta_eff = max_beta_eff,
        .a_eff = a_eff,
        .use_doubling = use_doubling,
        .b_start = b_start,
        .ndouble = ndouble,
        .zplus_value = zplus_value,
        .e_row = math.exp(-b_start / @max(row_mu, 1.0e-12)),
        .e_col = math.exp(-b_start / @max(col_mu, 1.0e-12)),
        .eet = eet,
        .dmu_min = dmu_min,
        .single_t_value = single_t_value,
        .final_t_value = T.get(row_index, column_index),
    };
}

pub fn calcTransmissionIterationProbes(
    layer: common.LayerInput,
    i_fourier: usize,
    row_index: usize,
    column_index: usize,
    ...
    var R = singleScatterR(a, &E, &z.Zmin, geo);
    var T = singleScatterT(a, b_start, &E, &z.Zplus, geo);
    return doDoubleWithProbe(ndouble, geo.nmutot, geo.n_gauss, controls.threshold_mul, geo, b_start, row_index, column_index, &R, &T, &E, probes);
}
