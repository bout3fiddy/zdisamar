...
//   lives in common/worker_partition.zig so cache and sampling builders can use the same canonical policy.    |
...
pub fn radianceAtWavelength(
    ...
    //                                                                                                         |
    // route                                                                                                   |
    //   One exact wavelength fills configured optical rows, runs LABOS transport, and scales reflectance      |
    //   into the prefetched radiance row.                                                                     |
    //                                                                                                         |
    // memory                                                                                                  |
    ...
    //                                                                                                         |
    // unsupported routes                                                                                      |
    //   Source-level and curved-sun rows are only filled for controls that request those routes. The          |
...
fn prefetchRadianceRows(
    ...
    //                                                                                                         |
    // scheduling                                                                                              |
    //   pool != null : workers share a ChunkQueue with chunk size 8, matching the pooled forward route        |
...
fn totalOpticalDepth(layers: []const layer_depths.LayerOptics) f64 {
    // totalOpticalDepth ------------------------------------------------------------------------------------  |
    // Reduce explicit layer rows into the scalar optical depth used by the direct-surface route.              |
    // --------------------------------------------------------------------------------------------------------|
    var total: f64 = 0.0;
...
pub fn gatherProductRows(
    ...
    //                                                                                                         |
    // route                                                                                                   |
    //   Dense radiance rows and solar lookup produce raw product rows; `instrument_average.zig` applies       |
...
pub fn postprocessAndAssembleProductRows(
    ...
    // Apply the product-grid postprocess order and final reflectance assembly to caller-owned rows.           |
    //                                                                                                         |
    ...
    // why: preserves visibility of the simulate.radiance_postprocess stage.                                   |
    {
    ...
    // why: preserves visibility of the simulate.irradiance_postprocess stage.                                 |
    {
...
pub fn preferredRadianceWorkerCount(radiance_count: usize) usize {
    ...
    // worker-count policy. This is the `preferredForwardWorkerCount` rule under the spectrum owner.           |
    // --------------------------------------------------------------------------------------------------------|
