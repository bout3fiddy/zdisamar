...
def main() -> None:
    ...
    operations = [
        ...
        (
            ...
            "smul_12x10",
            "threshold-skipped products are counted separately and do not run smul_12x10",
        ),
        (
            "D update",
            "matrix_smul_add_semul3",
            "smulAddSemul3KnownRightTrace_12",
            "T + Q*diag(E) + Q*T fused update with T trace reused by the doubling loop",
        ),
        (
            "U update with R*D",
            "matrix_semul_add",
        ...
        (
            "R_next update with T*U",
            "matrix_mat_add_esmul3",
            "matAddEsmul3_12",
            "R + diag(E)*U + T*U product result",
        ),
        (
            "R_next update without T*U",
            "matrix_mat_add_esmul",
            "matAddEsmul_12",
            "R + diag(E)*U when T*U is below threshold",
        ),
        (
            "T_next update with T*D",
            "matrix_esmul_semul_add",
            "esmulSemulAdd_12",
            "diag(E)*D + T*diag(E) + T*D product result",
        ),
        (
            "T_next update without T*D",
            "matrix_esmul_semul",
            "esmulSemul_12",
            "diag(E)*D + T*diag(E) when T*D is below threshold",
        ),
    ]
