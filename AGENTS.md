...
- Keep routines under `rust_src/forward_model/` free of file I/O, CLI wiring, text parsing, and hidden global state.
- Keep scientific assets under `data/reference_data/`; loaders and parsers live under `rust_src/input/reference_data.rs` and `rust_src/input/reference/`.
...
- Source-tree rules: [rust_src/AGENTS.md](rust_src/AGENTS.md).
...
- Fast baseline: `cargo test`.
- Broader fast presubmit: `cargo test` plus `cargo clippy --all-targets --all-features -- -D warnings`.
- Full retained verification: Rust tests plus the validation scripts in `validation/README.md`.
- Before committing: `prek run --all-files`.
- Validation evidence scripts are invoked directly with `uv run ...`; do not
  expose one-off validation plot or sweep scripts through Cargo.
