use crate::{
    common::errors,
    input::instrument::constants::{MAX_LINE_SHAPE_NOMINALS, MAX_LINE_SHAPE_SAMPLES},
};
...
impl BuiltinLineShapeKind {
    pub fn parse(name: &str) -> Result<Self, errors::Error> {
        match name {
...
impl InstrumentLineShape {
    pub fn validate(&self) -> Result<(), errors::Error> {
        ...
        for weight in self.weights.iter().take(sample_count) {
            if *weight < 0.0 {
                return Err(errors::Error::InvalidRequest);
            }
            weight_sum += weight;
...
impl InstrumentLineShapeTable {
    pub fn validate(&self) -> Result<(), errors::Error> {
        ...
        let sample_count = usize::from(self.sample_count);
        if nominal_count > MAX_LINE_SHAPE_NOMINALS || sample_count > MAX_LINE_SHAPE_SAMPLES {
            return Err(errors::Error::InvalidRequest);
        }
        if nominal_count == 0 && sample_count == 0 {
            return Ok(());
        }
        if nominal_count == 0 || sample_count == 0 {
            return Err(errors::Error::InvalidRequest);
        ...
        for nominal_index in 0..nominal_count {
            ...
            if previous_nominal.is_some_and(|previous| nominal < previous) {
                return Err(errors::Error::InvalidRequest);
            }
    ...
    pub fn set_weight(&mut self, nominal_index: usize, sample_index: usize, value: f64) {
        let index = nominal_index * usize::from(self.sample_count) + sample_index;
