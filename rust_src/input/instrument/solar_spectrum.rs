...
impl OperationalSolarSpectrum {
    ...
    pub fn validate(&self) -> Result<(), errors::Error> {
        if !self.enabled() {
            return if self.irradiance.is_empty() && self.spline_second_derivatives.is_empty() {
                Ok(())
            } else {
    ...
    pub fn prepare_interpolation(&mut self) -> Result<(), errors::Error> {
        self.validate()?;
        ...
        // DISAMAR uses its spline helper with first derivatives specified at both
        // ends for the O2 A solar source. Keeping the same setup avoids small but
        // visible last-bit differences in irradiance.
        c4[0] = 1.0;
        c3[0] = 0.0;
    ...
    pub fn clone_prepared(&self) -> Result<Self, errors::Error> {
        ...
        if cloned.spline_second_derivatives.is_empty() {
            cloned.prepare_interpolation()?;
        }
        Ok(cloned)
    }
    ...
    pub fn interpolate_irradiance(&self, wavelength_nm: f64) -> f64 {
        self.interpolate_irradiance_within_bounds(wavelength_nm)
            .unwrap_or_else(|| {
                if !self.enabled() {
                    0.0
                } else if wavelength_nm <= self.wavelengths_nm[0] {
                    self.irradiance[0]
                } else {
    ...
    pub fn interpolate_irradiance_linear_within_bounds(&self, wavelength_nm: f64) -> Option<f64> {
        if !self.enabled() || wavelength_nm < self.wavelengths_nm[0] {
            return None;
        }
        ...
        (wavelength_nm == self.wavelengths_nm[self.wavelengths_nm.len() - 1])
            .then(|| self.irradiance[self.irradiance.len() - 1])
    }
    ...
    fn interpolate_prepared_spline_within_bounds(&self, wavelength_nm: f64) -> Option<f64> {
        ...
        // DISAMAR evaluates prepared spline derivatives in Horner form. This
        // keeps reductions in the same order for parity with retained spectra.
