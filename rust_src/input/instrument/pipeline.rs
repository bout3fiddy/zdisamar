use crate::{
    common::errors,
    input::instrument::line_shape::{
        BuiltinLineShapeKind, InstrumentLineShape, InstrumentLineShapeTable,
    },
};
...
impl NodalCorrection {
    ...
    pub fn validate(&self) -> Result<(), errors::Error> {
        ...
        if self.wavelengths_nm.is_empty()
            ...
            || (!self.variances.is_empty() && self.variances.len() != self.values.len())
            || (!self.characteristic_bias.is_empty()
...
impl SinusoidalFeatures {
    ...
    pub fn validate(self) -> Result<(), errors::Error> {
        if !self.additive_amplitude_percent.is_finite()
            ...
            || (self.additive_amplitude_percent != 0.0 && self.additive_period_nm <= 0.0)
            || (self.multiplicative_amplitude_percent != 0.0
                && self.multiplicative_period_nm <= 0.0)
        {
            return Err(errors::Error::InvalidRequest);
        }
        Ok(())
...
impl NoiseControls {
    pub fn validate(&self) -> Result<(), errors::Error> {
        if !self.electrons_per_count.is_finite()
            || self.electrons_per_count <= 0.0
            || !self.reference_bin_width_nm.is_finite()
            || self.reference_bin_width_nm < 0.0
            || self.snr_max.is_nan()
            || self.snr_max <= 0.0
            || !self.lab_a.is_finite()
            || self.lab_a < 0.0
            || !self.lab_b.is_finite()
            || self.lab_b < 0.0
            || (self.enabled && self.model == NoiseModelKind::LabOperational && self.lab_a <= 0.0)
            || self.snr_wavelengths_nm.len() != self.snr_values.len()
...
impl RingControls {
    pub fn validate(&self) -> Result<(), errors::Error> {
        if !self.coefficient.is_finite()
            ...
            || self.degree_poly > 7
            || self.spectrum.iter().any(|value| !value.is_finite())
        {
            return Err(errors::Error::InvalidRequest);
        }
        Ok(())
