use crate::common::{
    errors::{Error, Result},
    units::{AltitudeRangeKm, PressureRangeHpa},
};

use super::types::{IntervalSemantics, ParticlePlacementSemantics};
...
impl IntervalGrid {
    ...
    pub fn validate(&self, fallback_sublayer_divisions: u8) -> Result<()> {
        if !self.enabled() {
        ...
        for (index, interval) in self.intervals.iter().enumerate() {
            ...
            if index != 0 {
                if !approx_eq_abs(interval.top_pressure_hpa, previous_bottom_pressure_hpa) {
                    return Err(Error::InvalidRequest);
                }
                // Tiny parser roundoff should not break an otherwise continuous grid.
                if previous_has_altitude_bounds
                    && interval.has_altitude_bounds()
                    && !approx_eq_abs(previous_bottom_altitude_km, interval.top_altitude_km)
                {
                    return Err(Error::InvalidRequest);
                }
            }
        ...
        if fallback_sublayer_divisions == 0 {
            return Err(Error::InvalidRequest);
        }
        Ok(())
    }
...
impl IntervalPlacement {
    ...
    pub fn validate(self) -> Result<()> {
        ...
        match self.semantics {
            ...
            ParticlePlacementSemantics::AltitudeCenterWidthApproximation => AltitudeRangeKm {
                bottom_km: self.bottom_altitude_km,
                top_km: self.top_altitude_km,
            }
            .validate()
...
fn approx_eq_abs(lhs: f64, rhs: f64) -> bool {
    (lhs - rhs).abs() <= 1.0e-9
}
