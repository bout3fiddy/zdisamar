use crate::{
    ...
    input::{AerosolType, IntervalPlacement, ParticlePlacementSemantics, Scene},
};
...
pub fn scale_optical_depth(
    ...
    if angstrom_exponent == 0.0 {
        return optical_depth;
    }
    if reference_wavelength_nm == wavelength_nm {
        return optical_depth;
    }
...
pub fn build_aerosol_sublayer_distribution(
    scene: &Scene,
    grid: PreparedVerticalGrid<'_>,
) -> Result<Vec<f64>> {
    let total_optical_depth = scene.aerosol.optical_depth;
    if scene.aerosol.placement.semantics == ParticlePlacementSemantics::ExplicitIntervalBounds {
        return build_placement_bound_distribution(
            grid,
    ...
    if scene.aerosol.aerosol_type == AerosolType::HgScattering {
        let placement = scene.aerosol.resolved_placement();
        return build_finite_layer_sublayer_distribution(
            grid,
            scene.atmosphere.has_aerosols && scene.aerosol.enabled && total_optical_depth > 0.0,
            total_optical_depth,
    ...
    build_gaussian_sublayer_distribution(
        grid,
        scene.atmosphere.has_aerosols && scene.aerosol.enabled && total_optical_depth > 0.0,
        total_optical_depth,
...
pub fn build_cloud_sublayer_distribution(
    scene: &Scene,
    grid: PreparedVerticalGrid<'_>,
) -> Result<Vec<f64>> {
    let total_optical_depth = scene.cloud.optical_thickness;
    let placement = scene.cloud.resolved_placement();
    if scene.cloud.placement.semantics == ParticlePlacementSemantics::ExplicitIntervalBounds {
        return build_placement_bound_distribution(
            grid,
            scene.atmosphere.interval_grid.enabled(),
            scene.atmosphere.has_clouds && scene.cloud.enabled && total_optical_depth > 0.0,
            total_optical_depth,
    ...
    build_finite_layer_sublayer_distribution(
        ...
        placement.top_altitude_km,
        !scene.atmosphere.interval_grid.enabled()
            && placement.semantics == ParticlePlacementSemantics::AltitudeCenterWidthApproximation,
    )
}
...
pub fn build_interval_matched_distribution(
    ...
    for ((slot, &parent_interval_index_1based), &support_weight_km) in weights
        .iter_mut()
        .zip(grid.sublayer_parent_interval_indices_1based)
        .zip(grid.sublayer_support_weights_km)
    {
...
pub fn build_finite_layer_sublayer_distribution(
    ...
    for (((slot, &slot_top_km), &slot_bottom_km), &support_weight_km) in weights
        .iter_mut()
        .zip(grid.sublayer_top_altitudes_km)
        .zip(grid.sublayer_bottom_altitudes_km)
        .zip(grid.sublayer_support_weights_km)
    {
...
pub fn build_gaussian_sublayer_distribution(
    ...
    for ((slot, &altitude_km), &support_weight_km) in weights
        .iter_mut()
        .zip(grid.sublayer_mid_altitudes_km)
        .zip(grid.sublayer_support_weights_km)
    {
