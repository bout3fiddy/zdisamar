use crate::{
    ...
    input::{NoiseModelKind, Scene, SpectralChannel},
};

pub use noise::Error;

pub type Result<T> = std::result::Result<T, Error>;
pub type MaterializesSigmaFn = fn(&Scene, SpectralChannel) -> bool;
pub type MaterializeSigmaFn = fn(&Scene, SpectralChannel, &[f64], &[f64], &mut [f64]) -> Result<()>;

#[derive(Debug, Clone, Copy)]
pub struct Implementation {
    pub id: &'static str,
    pub materializes_sigma: MaterializesSigmaFn,
    pub materialize_sigma: MaterializeSigmaFn,
}

pub fn resolve(provider_id: &str) -> Option<Implementation> {
    match provider_id {
        "builtin.scene_noise" => Some(Implementation {
            id: "builtin.scene_noise",
            materializes_sigma: scene_noise_enabled,
            materialize_sigma: scene_noise_sigma,
        }),
        "builtin.none_noise" => Some(Implementation {
            id: "builtin.none_noise",
            materializes_sigma: never_enabled,
            materialize_sigma: zero_sigma,
        }),
        "builtin.shot_noise" => Some(Implementation {
            id: "builtin.shot_noise",
            materializes_sigma: always_enabled,
            materialize_sigma: shot_noise_sigma,
        }),
        "builtin.s5p_operational_noise" => Some(Implementation {
