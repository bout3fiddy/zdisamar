use crate::input::{BuiltinLineShapeKind, SlitIndex, SpectralResponse};

use super::types::IntegrationKernel;
...
pub fn reset_kernel(kernel: &mut IntegrationKernel) {
    kernel.enabled = false;
    kernel.sample_count = 0;
    kernel.offsets_nm.fill(0.0);
    kernel.weights.fill(0.0);
}
...
pub fn spectral_response_weight(response: &SpectralResponse, offset_nm: f64) -> f64 {
    ...
    match response.slit_index {
        SlitIndex::GaussianModulated => {
            let sigma_nm = fwhm_nm / 2.354820045;
...
pub fn builtin_line_shape_weight(shape: BuiltinLineShapeKind, fwhm_nm: f64, offset_nm: f64) -> f64 {
    ...
    match shape {
        BuiltinLineShapeKind::Gaussian => {
            let sigma_nm = safe_fwhm_nm / 2.354820045;
...
pub fn flat_top_n4_weight(fwhm_nm: f64, offset_nm: f64) -> f64 {
    let w_nm = fwhm_nm / 1.681793;
    2.0_f64.powf(-2.0 * (offset_nm / w_nm.max(1.0e-6)).powi(4))
}
