...
pub fn shot_noise_std(signal: &[f64], electrons_per_count: f64, output: &mut [f64]) -> Result<()> {
...
pub fn whiten_residuals(residual: &[f64], sigma: &[f64], output: &mut [f64]) -> Result<()> {
...
pub fn copy_input_sigma(input_sigma: &[f64], output: &mut [f64]) -> Result<()> {
...
fn s5_operational_coefficients(wavelength_nm: f64) -> Result<S5Coefficients> {
