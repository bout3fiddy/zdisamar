...
impl From<sampling::Error> for Error {
    fn from(_: sampling::Error) -> Self {
        Self::ShapeMismatch
    }
}
...
pub fn apply_signal(calibration: Calibration, signal: &[f64], output: &mut [f64]) -> Result<()> {
...
pub fn apply_simple_offsets(offsets: SimpleOffsets, signal: &mut [f64]) -> Result<()> {
    if signal.is_empty() {
        return Ok(());
    }
...
pub fn apply_simple_offset_derivatives(offsets: SimpleOffsets, signal: &mut [f64]) -> Result<()> {
...
pub fn apply_smear(percent_smear: f64, signal: &mut [f64], scratch: &mut [f64]) -> Result<()> {
...
pub fn apply_stray_light_nodes(
    ...
    if correction.values.len() > node_values.len() {
        return Err(Error::ShapeMismatch);
    }
    for (index, slot) in node_values
        .iter_mut()
        .enumerate()
        .take(correction.values.len())
    {
        ...
        let bias = correction
            ...
            .copied()
            .unwrap_or(1.0);
        *slot = 0.01 * correction.values[index] * base * bias;
    }
...
pub fn apply_reflectance_calibration_error_sigma(
    ...
    if calibration_error.additive_error.enabled() {
        let mut node_values = [0.0; 64];
        if calibration_error.additive_error.values.len() > node_values.len() {
            return Err(Error::ShapeMismatch);
        }
        for (index, slot) in node_values
            .iter_mut()
            ...
            .take(calibration_error.additive_error.values.len())
        {
            ...
            *slot = reflectance_at_node * calibration_error.additive_error.values[index] / 100.0;
        }
...
fn sample_linear(x: &[f64], y: &[f64], target_x: f64) -> Result<f64> {
    Ok(sampling::sample_linear_clamped(x, y, target_x)?)
}

fn sample_polynomial(x: &[f64], y: &[f64], target_x: f64) -> Result<f64> {
