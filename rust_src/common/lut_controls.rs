...
impl CompatibilityKey {
    ...
    pub fn validate(self) -> Result<(), errors::Error> {
        ...
        if self.spectral_end_nm <= self.spectral_start_nm
            ...
            || self.instrument_line_fwhm_nm < 0.0
...
fn approx_eq_compatible_f64(lhs: f64, rhs: f64) -> bool {
    // Compatibility keys come from text and generated grids, so exact equality alone is brittle.
    lhs == rhs || (lhs - rhs).abs() <= 1.0e-12 || approx_eq_rel(lhs, rhs, 1.0e-12)
}

fn approx_eq_rel(lhs: f64, rhs: f64, tolerance: f64) -> bool {
    (lhs - rhs).abs() <= tolerance * lhs.abs().max(rhs.abs())
}
