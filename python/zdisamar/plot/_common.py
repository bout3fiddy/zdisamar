...
def active_profile_rows(
    ...
    *,
    ...
    finite = np.isfinite(result[vertical_axis].to_numpy(dtype=float)) & np.isfinite(result[value].to_numpy(dtype=float))
...
def interval_profile_rows(
    ...
    *,
    ...
    result = active_profile_rows(obj, value=value, vertical_axis=vertical_axis, wavelength_nm=wavelength_nm)
    require_columns(result, required)

    top_column = "top_pressure_hpa" if vertical_axis == "pressure_hpa" else "top_altitude_km"
    bottom_column = "bottom_pressure_hpa" if vertical_axis == "pressure_hpa" else "bottom_altitude_km"
    ...
    summed[vertical_axis] = 0.5 * (summed[top_column].to_numpy(dtype=float) + summed[bottom_column].to_numpy(dtype=float))
    if vertical_axis == "pressure_hpa":
...
def component_sums(obj: Any, components: Sequence[str], *, group_by: Sequence[str] = (fields.WAVELENGTH_NM,)):
    ...
    if melted.empty:
        return melted
    return melted.groupby([*group_by, "component", "component_label"], as_index=False)[fields.VALUE].sum()
