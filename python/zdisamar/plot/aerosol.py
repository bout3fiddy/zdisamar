...
def share_spectrum(
    ...
    *,
    ...
    numerator = {
        "total": fields.AEROSOL_OPTICAL_DEPTH,
        "scattering": "aerosol_scattering_optical_depth",
    ...
    return (
        alt.Chart(grouped)
        .mark_line(color=SEMANTIC_COLORS["aerosol"])
        .encode(
            x=alt.X(f"{fields.WAVELENGTH_NM}:Q", title=label(fields.WAVELENGTH_NM)),
            y=alt.Y("aerosol_share:Q", title=f"Aerosol {share} share"),
            tooltip=[
