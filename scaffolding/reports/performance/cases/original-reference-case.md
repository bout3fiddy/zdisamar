...
```text
At checkpoint X, the measured forward elapsed time moved from A to B.
```

Do not describe those values as the current retained elapsed time unless the linked
artifact is the current retained artifact.
