...
- `tests/validation/o2a_vendor_reflectance_profile_smoke_test.zig` is a separate opt-in smoke lane for profiled/unprofiled parity and profile-report generation. Run it with `zig build test-validation-o2a-vendor-profile` when you need that coverage.
