...
- The lane runs one Zig O2A forward spectrum with vendor-equivalent `Config_O2_with_CIA.in` controls, times the run, compares it against the cached vendor CSV, and emits a JSON report with the parity controls that were actually used.
- The parity fixture must keep the vendor transport and placement semantics intact: `20` LABOS streams, explicit pressure intervals, fit interval `2`, and aerosol placement in vendor interval `2`.
- Run it with `zig build test-validation-o2a-vendor -Doptimize=ReleaseFast` when you want the runtime-oriented O2A vendor-parity compare without invoking the vendored executable.
