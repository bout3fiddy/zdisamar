...
- [x] Which atmospheric interval contributes most to the aerosol signal? Output: interval-resolved aerosol optical-depth aggregation from the atmospheric budget table. Created in `src/output/atmospheric_budget.zig`, `python/zdisamar/native_tables.py`, and `scripts/testing_harness/python_atmosphere_budget.py`. Test: `zig build python-atmosphere-budget`.
