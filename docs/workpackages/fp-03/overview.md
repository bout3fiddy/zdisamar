...
| [WP-03](./wp-03-line-absorbing-spectroscopy-and-strong-line-sampling-parity.md) | In Progress | 2026-03-21 | `zig build test-unit --summary all` -> `45/45`; `zig build test-validation-o2a-adaptive`, `zig build test-validation-o2a-controls`, and `zig build test-validation-line-gas` all passed on the current line-gas validation slice | Finish the unfiltered O2A lane plus vendor-corpus SWIR/H2O-NH3 coverage and broader integration reruns |
