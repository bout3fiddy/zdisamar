...
| [WP-02](./wp-02-forward-transport-solver-parity.md) | In Progress | 2026-03-19 | `37/37` fast/unit gates pass; the transport source harness, targeted RTM-sensitive integration, compatibility-harness route check, O2A morphology gate, and the nonuniform plus `sublayer_divisions = 1` prepared-route RTM quadrature proofs all pass; prepared execution still threads a guarded pseudo-spherical attenuation grid from prepared sublayers into the live LABOS/adding attenuation path, LABOS `scattering = none` still uses resolved attenuation instead of bypassing the live path, and the latest promoted RTM carrier slice now uses per-parent-layer Gauss nodes and weights with zero-weight boundaries while conserving `sum(weight * ksca)` on the prepared adding path; independent correctness, vendor-context, and test/build auditors all passed that carrier promotion, and the bounded `300s` full gate still reports only the same unrelated `9` failures plus the known `BundledOptics` leak outside transport | Finish vendor-faithful adding consumption or replace the midpoint-derived interpolated RTM-node optics with a true vendor RTM-subgrid optics preparation before revisiting donor-sampled RTM-node optics, then continue pseudo-spherical parity before `WP-03` |
| [WP-03](./wp-03-line-absorbing-spectroscopy-and-strong-line-sampling-parity.md) | Todo | 2026-03-18 | Line-by-line / strong-line acceptance in `WP-09` | Wait for `WP-02` |
