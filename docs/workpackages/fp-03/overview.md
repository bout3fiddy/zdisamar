...
| [WP-05](./wp-05-atmospheric-intervals-aerosol-cloud-fraction-and-subcolumns-parity.md) | Done | 2026-03-25 | Explicit pressure-bounded interval grids, typed aerosol/cloud placement and fraction controls, prepared-optics provenance metadata, and subcolumn-partition parity checks are in place; final post-review hardening aligned explicit-grid workspace sizing, tightened monotonic fraction-grid validation, rejects targetless threshold-only fraction configs, infers retrieval HG aerosol and cloud optical-thickness inputs as `hg_scattering`, rejects interval-index particle placements without enabled interval grids, rejects pressure gaps plus non-monotonic adjacent explicit interval altitude bounds, restores legacy finite-layer padding and top-of-atmosphere clamping for non-interval particle placements, documents the bottom-up explicit-grid interval-identity contract, simplifies relative-azimuth normalization, and revalidated `zig build test-unit --summary all` -> `152/152`, `zig build test-integration-forward-model --summary all` -> `9/9`, `zig build test-validation-o2a --summary all` -> `6/6`, and `zig build test-validation-compatibility-full --summary all` -> `1/1` on the WP-05 branch state, while broader integration and validation red lanes were reproduced on clean `HEAD` and remain out of scope for this WP | Start `WP-06` |
| [WP-06](./wp-06-instrument-radiance-irradiance-slit-calibration-corrections-and-ring-parity.md) | Done | 2026-03-25 | Typed radiance/irradiance measurement pipelines, explicit correction and Ring staging, per-product sigma propagation, canonical-config instrument mapping, and post-review hardening for S5 bin-width fallback, default Ring coefficient handling, and LAB-operational validation are in place; `zig build test-validation --summary all` -> `47/47`, `zig build test-integration --summary all` -> `34/34`, and the latest `zig build check --summary all` -> `154/154` on the WP-06 branch state | Start `WP-07` |
| [WP-07](./wp-07-operational-measured-input-and-s5p-interface-parity.md) | Done | 2026-03-28 | Typed measured-input ingestion, explicit `operational_measured_input` request/plan/result provenance, S5P operational bridge, aggregate transport wiring, compatibility classification, and final post-review hardening for malformed inert operational HR-grid controls plus duplicate operational carrier removal landed; `zig build test-unit --summary all` -> `161/161`, `zig build test-transport --summary all` -> `180/180`, `zig build test-validation --summary all` -> `48/48`, `zig build test-fast --summary all` -> `196/196`, and `zig build check --summary all` -> `161/161` on the WP-07 branch state | Start `WP-08` |
| [WP-08](./wp-08-lut-and-xseclut-generation-consumption-and-cache-parity.md) | Done | 2026-03-28 | Typed LUT/XsecLUT controls, compatibility-keyed cache registration, generated/consume provenance, and focused O2A plus non-O2 LUT parity are in place; `zig build test-unit --summary all` -> `165/165`, `zig build test-validation-lut-assets --summary all` -> `2/2`, `zig build test-validation-compatibility-lut-parity --summary all` -> `2/2`, `zig build test-fast --summary all` -> `200/200`, and `zig build check --summary all` -> `165/165` on the WP-08 branch state | Start `WP-09` |
