...
| [WP-05](./wp-05-atmospheric-intervals-aerosol-cloud-fraction-and-subcolumns-parity.md) | Done | 2026-03-25 | Explicit pressure-bounded interval grids, typed aerosol/cloud placement and fraction controls, prepared-optics provenance metadata, and subcolumn-partition parity checks are in place; final post-review hardening aligned explicit-grid workspace sizing, tightened monotonic fraction-grid validation, rejects targetless threshold-only fraction configs, rejects interval-index particle placements without enabled interval grids, rejects non-monotonic adjacent explicit interval altitude bounds, documents the bottom-up explicit-grid interval-identity contract, simplifies relative-azimuth normalization, and revalidated `zig build test-unit --summary all` -> `149/149`, `zig build test-integration-forward-model --summary all` -> `9/9`, `zig build test-validation-o2a --summary all` -> `6/6`, and `zig build test-validation-compatibility-full --summary all` -> `1/1` on the WP-05 branch state, while broader integration and validation red lanes were reproduced on clean `HEAD` and remain out of scope for this WP | Start `WP-06` |
